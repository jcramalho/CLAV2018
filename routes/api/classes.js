    ...
    try {
      ...
      if (req.query.info) {
        if (req.query.info == "esqueleto") {
          ...
          let ents_tips = ents.concat(tips);
          res.locals.dados = State.filterPreSelecionadoInfo(
            res.locals.dados,
            ents_tips
          );
      ...
      if (
        ...
        res.locals.dados = State.filterStatus(
          res.locals.dados,
          res.locals.dados
        );
      }
...
router.get("/titulo", Auth.isLoggedInKey, [existe("query", "valor")], function (
  req,
  res
) {
  const errors = validationResult(req);
...
router.get(
  "/:id",
  Auth.isLoggedInKey,
  [verificaId(), eFS(), estaEm("query", "tipo", vcClasseInfo).optional()],
  async function (req, res, next) {
    ...
    try {
      ...
      res
        .status(500)
        .send(`Erro na recuperação da classe ` + req.params.id + `: ${err}`);
    }
  }
);

// Devolve a metainformação de uma classe: codigo, titulo, status, desc, codigoPai?, tituloPai?, procTrans?, procTipo?
router.get("/:id/meta", Auth.isLoggedInKey, [verificaId()], function (
  req,
  res
) {
  const errors = validationResult(req);
...
router.get("/:id/descendencia", Auth.isLoggedInKey, [verificaId()], function (
  req,
  res
) {
  const errors = validationResult(req);
...
router.get("/:id/notasAp", Auth.isLoggedInKey, [verificaId()], (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(422).jsonp(errors.array());
  }
  ...
  Classes.notasAp(req.params.id)
    ...
    .catch(erro =>
      res
        .status(500)
        .send(
          `Erro na consulta das notas de aplicação da classe ${req.params.id}: ${erro}`
        )
    );
});
    ...
    Classes.exemplosNotasAp(req.params.id)
      ...
      .catch(erro =>
        res
          .status(500)
          .send(
            `Erro na consulta dos exemplos das notas de aplicação da classe ${req.params.id}: ${erro}`
          )
      );
  }
);
...
router.get("/:id/notasEx", Auth.isLoggedInKey, [verificaId()], (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(422).jsonp(errors.array());
  }
  ...
  Classes.notasEx(req.params.id)
    ...
    .catch(erro =>
      res
        .status(500)
        .send(
          `Erro na consulta das notas de exclusão da classe ${req.params.id}: ${erro}`
        )
    );
});

// Devolve os termos de índice de uma classe: idTI, termo
router.get("/:id/ti", Auth.isLoggedInKey, [verificaId()], (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(422).jsonp(errors.array());
  }
  ...
  Classes.ti(req.params.id)
    ...
    .catch(erro =>
      res
        .status(500)
        .send(
          `Erro na consulta dos termos de índice da classe ${req.params.id}: ${erro}`
        )
    );
});

// Devolve a(s) entidade(s) dona(s) do processo: id, tipo, sigla, designacao
router.get("/:id/dono", Auth.isLoggedInKey, [verificaId()], (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(422).jsonp(errors.array());
  }
  ...
  Classes.dono(req.params.id)
    ...
    .catch(erro =>
      res
        .status(500)
        .send(`Erro na consulta dos donos da classe ${req.params.id}: ${erro}`)
    );
});
    ...
    Classes.participante(req.params.id)
      ...
      .catch(erro =>
        res
          .status(500)
          .send(
            `Erro na consulta dos participantes da classe ${req.params.id}: ${erro}`
          )
      );
  }
);
...
router.get("/:id/procRel", Auth.isLoggedInKey, [verificaId()], (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(422).jsonp(errors.array());
  }
  ...
  Classes.procRel(req.params.id)
    ...
    .catch(erro =>
      res
        .status(500)
        .send(
          `Erro na consulta dos processos relacionados com a classe ${req.params.id}: ${erro}`
        )
    );
});
...
router.get(
  "/:id/procRel/:idRel",
  Auth.isLoggedInKey,
  [verificaId(), estaEm("param", "idRel", vcClassesRels)],
  (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(422).jsonp(errors.array());
    ...
    Classes.legislacao(req.params.id)
      ...
      .catch(erro =>
        res
          .status(500)
          .send(
            `Erro na consulta da legislação associada à classe ${req.params.id}: ${erro}`
          )
      );
  }
);
...
router.get("/:id/pca", Auth.isLoggedInKey, [verificaId()], (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(422).jsonp(errors.array());
  }
  ...
  Classes.pca(req.params.id)
    ...
    .catch(erro =>
      res
        .status(500)
        .send(
          `Erro na consulta do PCA associado à classe ${req.params.id}: ${erro}`
        )
    );
});
  ...
  (req, res) => {
    ...
    Classes.justificacao(req.params.id)
      ...
      .catch(erro =>
        res
          .status(500)
          .send(`Erro na consulta da justificação ${req.params.id}: ${erro}`)
      );
  }
);

// Devolve a informação base do DF: idDF, valor, idJustificacao
router.get("/:id/df", Auth.isLoggedInKey, [verificaId()], (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(422).jsonp(errors.array());
  }
  ...
  Classes.df(req.params.id)
    ...
    .catch(erro =>
      res
        .status(500)
        .send(
          `Erro na consulta do DF associado à classe ${req.params.id}: ${erro}`
        )
    );
});
