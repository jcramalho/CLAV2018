...
router.get( "/:id", [verificaId(), eFS(), estaEm("query", "tipo", vcClasseInfo).optional()],
...
router.get("/:id/notasEx", [verificaId()], (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(422).jsonp(errors.array());
  }
...
router.get("/:id/ti", [verificaId()], (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(422).jsonp(errors.array());
  }
...
router.get("/:id/dono", [verificaId()], (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(422).jsonp(errors.array());
  }
...
router.get("/:id/participante", [verificaId()], (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(422).jsonp(errors.array());
    }
...
router.get("/:id/procRel", [verificaId()], (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(422).jsonp(errors.array());
  }
...
router.get( "/:id/procRel/:idRel", [verificaId(), estaEm("param", "idRel", vcClassesRels)],
  (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(422).jsonp(errors.array());
...
router.get( "/:id/legislacao", [verificaId()], (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(422).jsonp(errors.array());
    }
...
router.get("/:id/pca", [verificaId()], (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(422).jsonp(errors.array());
  }
...
router.get( "/justificacao/:id", [verificaJustId("param", "id")], (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(422).jsonp(errors.array());
    }
    ...
    Classes.justificacao(req.params.id)
      .then(dados => res.jsonp(dados))
      .catch(erro =>
        res.status(500).send(`Erro na consulta da justificação ${req.params.id}: ${erro}`)
      );
  }
);

// Devolve a informação base do DF: idDF, valor, idJustificacao
router.get("/:id/df", [verificaId()], (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(422).jsonp(errors.array());
  }
