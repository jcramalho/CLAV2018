...
router.get("/:id", Auth.isLoggedInKey, async (req, res, next) => {
  try {
    ...
    res.locals.dados
      ? next()
      : res.status(404).send(`Erro. A tipologia '${req.params.id}' não existe`);
  } catch (erro) {
    res
      .status(500)
      .send(`Erro na consulta da tipologia '${req.params.id}': ${erro}`);
  }
});
  ...
  return Tipologias.elementos(req.params.id)
    ...
    .catch(erro =>
      res
        .status(500)
        .send(
          `Erro na consula dos elementos da tipologia '${req.params.id}': ${erro}`
        )
    );
});
...
router.post("/", Auth.isLoggedInUser, Auth.checkLevel(4), (req, res) => {
  if (req.body && req.body.sigla && req.body.designacao) {
    Tipologias.criar(req.body)
      .then(dados => res.jsonp(dados))
      .catch(err =>
        res
          .status(500)
          .send(`Erro na inserção de uma tipologia de entidade: ${err}`)
      );
  } else {
    res
      .status(500)
      .send(
        "O seu pedido não possui todos os parâmetros necessários (sigla e designacao)."
      );
  }
});
