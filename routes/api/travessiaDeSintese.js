var Trav = require('../../controllers/travessiaDeSintese.js');
...
router.get('/', async (req, res) => {
    try {
        ...
        res.status(500).send(`Erro no carregamento das travessias de sintese: ${error}`)
    }
})
...
router.get('/reset', async function(req,res){
    try {
        ...
        res.status(500).send(`Erro ao fazer reset das travessias de sintese: ${err}`)
    }
})
...
router.get('/:id', [
    ...
    try {
        ...
        res.status(500).send(`Erro na travessia de sintese do processo ${req.params.id}: ${err}`)
    }
})
...
router.post('/', [
    ...
    try {
        ...
        res.status(500).send(`Erro na criação de uma nova travessia de sintese: ${err}`)
    }
})
