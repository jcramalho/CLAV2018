...
router.get("/", Auth.isLoggedInKey, async (req, res, next) => {
  ...
  if (
    queryData.estado &&
    (queryData.estado == "Ativo" || queryData.estado == "Revogado")
  ) {
    try {
  ...
  // api/legislacao?processos=com
    ...
    try {
      ...
      res
        .status(500)
        .send(`Erro na listagem dos diplomas com PNs associados: ${erro}`);
    }
  }
  // api/legislacao?processos=sem
    ...
    try {
      ...
      res
        .status(500)
        .send(`Erro na listagem dos diplomas sem PNs associados: ${erro}`);
    }
  }
  // api/legislacao?fonte=XXXX
    ...
    try {
      ...
      res
        .status(500)
        .send(
          `Erro na listagem de legislações com fonte ${queryData.fonte}: ${erro}`
        );
    }
  } else {
    try {
...
router.get("/portarias", Auth.isLoggedInKey, (req, res) => {
  return Leg.portarias()
    .then(dados => res.jsonp(dados))
    .catch(erro =>
      res.status(500).send(`Erro na consulta da leg de portarias: ${erro}`)
    );
});
...
router.get("/:id", Auth.isLoggedInKey, async (req, res, next) => {
  try {
    ...
    res.locals.dados
      ...
      : res
          .status(404)
          .send(`Erro. A legislação '${req.params.id}' não existe`);
  } catch (erro) {
  ...
  return Leg.regula(req.params.id)
    ...
    .catch(erro =>
      res
        .status(500)
        .send(
          `Erro na consulta dos processos regulados por ${req.params.id}: ${erro}`
        )
    );
});
...
router.post("/", Auth.isLoggedInUser, Auth.checkLevel(4), (req, res) => {
  if (
    ...
    Leg.criar(req.body)
      .then(dados => res.jsonp(dados))
      .catch(err =>
        res.status(500).send(`Erro na inserção de uma legislação: ${err}`)
      );
  } else {
    res
      .status(500)
