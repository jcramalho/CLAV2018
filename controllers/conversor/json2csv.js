...
function json2csvArray(name, array){
    ...
    if(len){
        if(array[0] instanceof Array){
            ...
            array.forEach((arr, index) => accum = accum.concat(json2csvRec(header + index, arr)))
...
function json2csvRec(name,json){
    ...
    if(json instanceof Array){
        csvLines = csvLines.concat(json2csvArray(name, json))
    }else{
        for(var key in json){
            var header = name != '' ? name + '.' + key : key
            ...
            if(json[key] instanceof Array){
                ...
                if(type === 'object' || type === 'function'){
                    var l = json2csvRec(header, json[key])
                    csvLines = csvLines.concat(l)
                }else{
